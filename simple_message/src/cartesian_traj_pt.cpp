...
bool CartesianTrajPt::load(industrial::byte_array::ByteArray *buffer)
{
  ...
  if (!buffer->load(this->acceleration_))
  {
    LOG_ERROR("Failed to load cartesian traj. pt. acceleration data");
    return false;
  }

  if (!buffer->load(this->blending_radius_))
  {
    LOG_ERROR("Failed to load cartesian traj. pt. blending radius data");
    return false;
  }
...
bool CartesianTrajPt::unload(industrial::byte_array::ByteArray *buffer)
{
  ...
  {
    LOG_ERROR("Failed to unload cartesian traj. pt. blending radius data");
    return false;
  }

  if (!buffer->unload(this->acceleration_))
  {
    LOG_ERROR("Failed to unload cartesian traj. pt. acceleration data");
    return false;
  }
