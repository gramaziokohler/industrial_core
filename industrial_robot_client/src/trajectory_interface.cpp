...
typedef industrial::cartesian_traj_pt::CartesianTrajPt rbt_CartesianTrajPt;
typedef trajectory_msgs::JointTrajectoryPoint ros_JointTrajPt;
typedef industrial_msgs::CartesianTrajectoryPoint ros_CartesianTrajPt;
...
class GetTimeFromStartVisitor : public boost::static_visitor<double>
{
public:
  template <typename T>
  double operator()(const T &pt) const
  {
    return pt.time_from_start.toSec();
  }
};

bool TrajectoryInterface::init(std::string default_ip, int default_port)
{
  std::string ip;
  int port;
  ...
  ROS_INFO("Trajectory Interface connecting to IP address: '%s:%d'", ip_addr, port);
...
bool TrajectoryInterface::init(SmplMsgConnection* connection)
{
                               ...
                               const std::map<std::string, double> &velocity_limits)
{
  ...
  this->srv_cartesian_trajectory_ = this->node_.advertiseService("cartesian_path_command", &TrajectoryInterface::cartesianTrajectoryCB, this);
  this->sub_joint_trajectory_ = this->node_.subscribe("joint_path_command", 0, &TrajectoryInterface::jointTrajectoryCB, this);
  this->sub_cartesian_trajectory_ = this->node_.subscribe("cartesian_path_command", 0, &TrajectoryInterface::cartesianTrajectoryCB, this);
  this->sub_cur_pos_ = this->node_.subscribe("joint_states", 1, &TrajectoryInterface::jointStateCB, this);
...
{  
  ...
  this->sub_cartesian_trajectory_.shutdown();
}

bool TrajectoryInterface::jointTrajectoryCB(industrial_msgs::CmdJointTrajectory::Request &req,
...
bool TrajectoryInterface::cartesianTrajectoryCB(industrial_msgs::CmdCartesianTrajectory::Request &req,
                                                industrial_msgs::CmdCartesianTrajectory::Response &res)
{
  industrial_msgs::CartesianTrajectoryPtr traj_ptr(new industrial_msgs::CartesianTrajectory);
  *traj_ptr = req.trajectory;  // copy message data
  this->cartesianTrajectoryCB(traj_ptr);

  // no success/fail result from cartesianTrajectoryCB.  Assume success.
  res.code.val = industrial_msgs::ServiceReturnCode::SUCCESS;

  return true;  // always return true.  To distinguish between call-failed and service-unavailable.
}

void TrajectoryInterface::jointTrajectoryCB(const trajectory_msgs::JointTrajectoryConstPtr &msg)
{
...
void TrajectoryInterface::cartesianTrajectoryCB(const industrial_msgs::CartesianTrajectoryConstPtr &msg)
{
...
bool TrajectoryInterface::select(const std::vector<std::string>& ros_joint_names, const ros_JointTrajPt& ros_pt,
...
bool TrajectoryInterface::calc_velocity(const trajectory_msgs::JointTrajectoryPoint& pt, double* rbt_velocity)
{
...
bool TrajectoryInterface::calc_duration(const boost::variant<trajectory_msgs::JointTrajectoryPoint, industrial_msgs::CartesianTrajectoryPoint> &pt, double *rbt_duration)
{
  double this_time = boost::apply_visitor(GetTimeFromStartVisitor(), pt);
...
JointTrajPtMessage TrajectoryInterface::create_message(int seq, std::vector<double> joint_pos, double velocity, double duration)
{
...
void TrajectoryInterface::trajectoryStop()
{
  JointTrajPtMessage jMsg;
  SimpleMessage msg, reply;

  ROS_INFO("Trajectory handler: entering stopping state");
...
bool TrajectoryInterface::stopMotionCB(industrial_msgs::StopMotion::Request &req,
...
bool TrajectoryInterface::is_valid(const trajectory_msgs::JointTrajectory &traj)
{
...
bool TrajectoryInterface::is_valid(const industrial_msgs::CartesianTrajectory &traj)
{
  for (int i = 0; i < traj.points.size(); ++i)
  {
    const industrial_msgs::CartesianTrajectoryPoint &pt = traj.points[i];
...
void TrajectoryInterface::jointStateCB(const sensor_msgs::JointStateConstPtr &msg)
{
